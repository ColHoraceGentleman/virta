...
**Owner:** Rusty
**Status:** Decisions locked 2026-07-07. **Round 2 applied 2026-07-08** (merged Owner + Business identity + Tax IDs into one step, alphabetical account numbering, NAICS lookup modal, edit-on-review pattern, asset/liability/equity subheaders, Welcome-screen Schedule C explainer). **Round 3 applied 2026-07-08** (stripped step 1, renamed Your Name, fixed step 6 UX). **Round 4 applied 2026-07-08** (Chantelle-specific placeholders removed; Categories wizard step 2 got Hide/Delete + sticky header + sortable columns + "show account numbers" wizard prompt; step 3 reordered (Sales first, Other Income last); step 4 unified to single Add Account button; Add modal got Type picker + Note field + relabeled "Tax Line Item (Schedule C of IRS Form 1040)"). **Round 5 applied 2026-07-08** (single-page Categories Management with search + 4 filter chips + Show hidden + new Settings → Categories subsection). **Round 6 applied 2026-07-08** (welcome uses checkbox not toggle; IRS Form 1040 introduced before Schedule C; step 2 cells got Edit + Hide + Delete with no inline rename + Category Name + Tax Description + Review Later moved fully off step 2 to step 5; "Skip" button became "Revert to Defaults" when state has changed; edit modal got a generic Notes field; Delete closure bug fixed; step 2 window taller; sticky-header rule confirmed global for in-wizard and management lists). **Round 15 applied 2026-07-09** (Phase 1 cleanup: spec dedupe of D29–D32/D43–D49, wireframe dead-code removal of `state.activeTab` + legacy `catFilter='revenue'/'ale'` router, smoke test updated for the D62 New-entry modal). **Round 16 applied 2026-07-09** (sidebar GL renamed to **Transactions** to match user expectation; GL functionality unchanged). **Round 17 applied 2026-07-09** (default landing screen changed from Settings to Dashboard; sidebar active-state moved accordingly). **Round 18 applied 2026-07-09** (manual-entry modal: Type picker is the FIRST field, filters the Account list, and drives the Change label + helper copy per type — D62 + D64 revised; sign convention D63 unchanged). **Round 19 applied 2026-07-09** (manual-entry Description field uses a placeholder, not a pre-filled "Owner draw adjustment" default value). **Round 20 applied 2026-07-09** (manual-entry: clear stale defaults across Change, Date; Type defaults to Expense, Date defaults to today at modal-open time, Change and Description are blank placeholders). **Round 21 applied 2026-07-09** (manual-entry modal: "Other account" picker renamed to "Matched with" for consistency with the GL table column (D59)). **Round 22 applied 2026-07-09** (manual-entry modal: "Account" picker renamed to "Category" for consistency with the GL table column (D59); modal Account row restructured so the static label is inline in the template, not filled by setTimeout). **Round 23 applied 2026-07-09** (manual-entry modal: added **Name** field to match the GL table column; D62 / D64 updated; "Change" is not a field name — it is part of the type-aware Amount label for Asset / Liability / Equity; **Notes** is internal-only, not in the GL table). **Round 24 applied 2026-07-09** (manual-entry modal: simplified D64 — Amount label is always "Amount"; type-aware copy lives only in the helper text under the field; "Change in the Asset" / "Change in the Liability" / "Change in Owner's Equity" labels removed). **Round 25 applied 2026-07-09** (manual-entry modal layout overhaul: bug fix for empty Category dropdown (removed setTimeout race), modal body now scrollable with sticky footer (Save button reachable), field order: Date → Type → Category → Name → Amount → Description → Matched with → Notes; Notes labeled "(internal only)"; single-column flex layout instead of 2-column grid; modal uses 90vh max-height with overflow-y auto on .body). **Round 26 applied 2026-07-09** (manual-entry modal redesign per Patrick's "kind of sucks" feedback, after Lore's UX research): default view collapsed from 8 fields to **5** (Date, Type, Category, Name, Amount); the other 3 fields (Description, Matched with, Notes) hide behind FreshBooks-style **+ Add X** links and have inline **remove** links to collapse back; new **Save and new** button in the sticky footer alongside Save and Cancel (D71); Sage-style yellow contextual warning fires under Matched with when the user picks an import-driven account (Credit Card / Checking / Savings / Bank / Stripe / PayPal / Venmo / Square / Plaid / Import) — D70. D62 revised to lock the 5-field default view + collapse pattern.
...
| D62 | v2 manual-entry form (button text: **New entry**) has **eight fields total**, but the **default view shows only 5** (revised 2026-07-09, R26): **Date** + **Type** + **Category** + **Name** + **Amount**. The remaining three — **Description**, **Matched with**, and **Notes** — are collapsed by default and exposed behind FreshBooks-style **`+ Add X`** links (`+ Add description`, `+ Add Matched with`, `+ Add note`). Clicking an `+ Add X` link expands the field inline; an inline **remove** link (right-aligned, muted) on each expanded field collapses it back. **Type is the first user-pickable field** (revised 2026-07-09, R18): the user picks Expense / Income / Asset / Liability / Equity first. Picking a Type (a) filters the Category dropdown to only that type, and (b) sets the Amount field's label and helper copy to the wording for that type (D64). **No debit/credit picker, no drafts.** The Category picker lets the user choose any account within the picked Type; the Matched with picker lets the user choose any account (defaults to the user's default cash account from Setup Wizard, can be overridden). **Name** (R23) is the vendor (for expenses) or customer (for income); optional. **Notes** (R23) is internal-only, not in the GL table; only visible when the user opens transaction details. |
...
| D64 | The Amount field's **label is always "Amount"** (revised 2026-07-09, R24). The type-aware explanation lives in the **helper copy** under the field, driven by the picked **Type**: **Expense** → "Positive = You spent this much. Negative = You got a refund (or a negative expense)."; **Income** → "Positive = You earned this much. Negative = You had a reversal."; **Asset** → "Positive = The asset went up. Negative = The asset went down."; **Liability** → "Positive = You paid it down. Negative = You took on more debt."; **Equity** → "Positive = Owner took money out. Negative = Owner put money in.". The helper copy changes immediately when the user picks a different Type. Sign convention (D63) stays: positive = the picked Category went up; negative = it went down. |
| D65 | Manual-entry form has a **single Save action** (no Save draft / Post entry split per D61). On save, the entry posts to the GL immediately, becomes visible everywhere (reports, dashboard), and is included in all balance/total calculations from that moment. Reconciliation status starts as empty (finalized when Phase 9 lands). *The footer may also contain a secondary **Save and new** action per D71 — that's a fast-path variant of the same Save action (post + reset form), not a draft split.* |
...
| D69 | v2 default landing screen on first open (or after a sidebar reset) is **Dashboard** (not Settings). Sidebar Dashboard link is highlighted `class="active"` to match. Rationale: Dashboard is the home base — it shows review-queue counts, upcoming bills, recent activity. Settings is a destination, not a starting point. |
| D70 | When the user expands the **Matched with** field in the manual-entry modal (D62) AND picks an account whose name (case-insensitive substring) contains any of these import-driven tokens — **credit card, checking, savings, bank, stripe, paypal, venmo, square, plaid, import** — a Sage-style **yellow contextual warning** appears under the field, in the same visual treatment as the existing "manual accounting adjustment" softwarn at the bottom of the modal. Copy: **"Heads up: This account is usually updated by statement imports. A manual entry will create a separate transaction that you will need to reconcile against the import later."** If the user picks a non-import-driven account (or no account is picked yet), the warning is hidden. The check re-runs on `onchange` of the Matched with select, so changing the selection updates the warning in real time. Rationale: an accountant seeing this warning knows the entry will need reconcile work; a non-accountant learns *why* the manual entry is special without having to remember which accounts are import-driven. |
| D71 | The manual-entry modal footer has **three buttons**, in this order from left to right: **Save and new** (secondary), **Cancel** (secondary), **Save** (primary). **Save** posts the entry to the GL (Phase 2 architecture; placeholder for now) and closes the modal. **Save and new** posts the entry, then **resets the form** (clears Amount, Name, Description, Matched with, Notes; collapses Description / Matched with / Notes back to their `+ Add X` links; keeps Type and Date at their current values) and **leaves the modal open** so the user can keep typing. **Cancel** closes the modal without saving. The primary visual style is applied via the existing `__openModal` convention: the Save button's label contains the word `primary`, so it gets `class="primary"` automatically. |
