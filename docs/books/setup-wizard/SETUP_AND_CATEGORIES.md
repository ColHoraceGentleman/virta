...
| D62 | v2 manual-entry form (button text: **New entry**) has four fields: **Type** + **Account** + **Change** + **Other account** + **Date** + **Description** + **Notes**. **Type is the first field** (revised 2026-07-09, R18): the user picks Expense / Income / Asset / Liability / Equity first. Picking a Type (a) filters the Account dropdown to only that type, and (b) sets the Change field's label and helper copy to the wording for that type. **No debit/credit picker, no drafts.** The Account picker lets the user choose any account within the picked Type; the Other account picker lets the user choose any account (defaults to the user's default cash account from Setup Wizard, can be overridden). |
...
| D64 | The Change field's label and helper copy are driven by the picked **Type** (not by the picked Account's type, per the R18 revision): **Expense** → "Amount of Expense" (positive = "You spent this much", negative = "You got a refund"); **Income** → "Amount of Income" (positive = "You earned this much", negative = "You had a reversal"); **Asset** → "Change in the Asset" (positive = "The asset went up", negative = "The asset went down"); **Liability** → "Change in the Liability" (positive = "You paid it down", negative = "You took on more debt"); **Equity** → "Change in Owner's Equity" (positive = "Owner took money out", negative = "Owner put money in"). The label and copy change immediately when the user picks a different Type. Sign convention (D63) stays: positive = the picked Account went up; negative = it went down. |
